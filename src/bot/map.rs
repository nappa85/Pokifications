...
fn latlon2relative_xy(lat: f64, lon: f64) -> (f64, f64) {
    ...
    let y = (1.0
        - (lat.to_radians().tan() + sec(lat.to_radians())).log(std::f64::consts::E)
            / std::f64::consts::PI)
        / 2.0;
    // return x, y
    (x, y)
...
impl<'a> Map<'a> {
    ...
    pub async fn get_map(
        &self,
        marker: Option<image::DynamicImage>,
        ...
        let lat_center_diff =
            ((x_absolute - (x_offset as f64)) * (self.tile_width as f64)).round() as i32;
        // lon_center_diff = int((y_absolute - y_offset) * self.tile_height)
        let lon_center_diff =
            ((y_absolute - (y_offset as f64)) * (self.tile_height as f64)).round() as i32;
        ...
        let mut image =
            image::DynamicImage::ImageRgba8(image::RgbaImage::new(image_width, image_height));
        ...
        for (row_offset, row) in tiles.into_iter().enumerate() {
            ...
            for (col_offset, (x, y)) in row.into_iter().enumerate() {
                ...
                image::imageops::replace(
                    &mut image,
                    new_image,
                    (col_offset as u32) * self.tile_width,
                    (row_offset as u32) * self.tile_height,
                );
            }
        }
    ...
    async fn get_tile(&self, x: i64, y: i64) -> Result<image::DynamicImage, ()> {
        ...
        let url = reqwest::Url::parse(&tile_url)
            .map_err(|e| tracing::error!("error building tile url: {}", e))?;
        ...
        if !res.status().is_success() {
            tracing::error!(
                "tile {} retriever failed with status code {}",
                tile_url,
                res.status()
            );
            Err(())
